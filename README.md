...
## Features
  ...
  * Password and key passphrase input via hidden prompt, environment variable, or CLI argument
...
Username and one SSH authentication method are mandatory. By default the script uses password authentication and prompts securely if no password is supplied through an environment variable or CLI option. Use `--key-file` to authenticate with an SSH private key instead.
...
Encrypted private key with passphrase prompt:

    ./linux_lpe_exposure_scan.py 10.0.10.0/24 -u audituser --key-file ~/.ssh/id_ed25519 --ask-key-passphrase

Encrypted private key with passphrase from environment variable:
...
## Options
  ...
  * `--key-file`: SSH private key file for key-based authentication. When set, password authentication is not used.
  * `--key-passphrase-env`: Environment variable containing the SSH private key passphrase. Default lookup is `LPE_SCAN_KEY_PASSPHRASE`.
  * `--key-passphrase`: SSH private key passphrase as an argument. Use the prompt or environment variable instead when shell history matters.
  * `--ask-key-passphrase`: Prompt for the SSH private key passphrase. Leave empty for an unencrypted key.
